<script>
    ...
    export let subtitle = "Coresynq bridges the gap between raw environmental data and actionable ESG insights.";
    ...
    const nodes = [
        { id: 'satellite', label: 'Satellite Data', x: 0, y: 0, color: '#e3f2fd', textColor: '#0071e3', icon: '🛰️', baseAngle: 0 },
        { id: 'grid', label: 'Energy Grid', x: 0, y: 0, color: '#e8f5e9', textColor: '#2e7d32', icon: '⚡', baseAngle: (Math.PI * 2) / 4 },
        { id: 'core', label: 'Coresynq AI', x: 0, y: 0, color: '#f3e5f5', textColor: '#7b1fa2', isCenter: true, icon: '🧠', baseAngle: 0 },
        { id: 'gri', label: 'GRI Framework', x: 0, y: 0, color: '#fff3e0', textColor: '#ef6c00', icon: '📋', baseAngle: (Math.PI * 2) / 2 },
        { id: 'sasb', label: 'SASB Standards', x: 0, y: 0, color: '#fce4ec', textColor: '#c2185b', icon: '🛡️', baseAngle: (Math.PI * 3 * 2) / 4 },
    ];
...
<style>
    .dynamic-nodes-wrapper {
        ...
        overflow: hidden;
        margin-top: -100px; /* Slight overlap with section above if needed */
    }
